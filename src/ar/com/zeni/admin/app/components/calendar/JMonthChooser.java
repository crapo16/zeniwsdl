...
public class JMonthChooser extends JPanel implements ItemListener,
	 ...
	 * @since 2010, spinner is mandatory
	 */
	public JMonthChooser(boolean hasSpinner, String[] monthNames) {
		super();
		hasSpinner = true;
		...
		// comboBox.setEnabled(false); //R-AAG-881

		// comboBox.addPopupMenuListener(this); //R-AAG-881
		locale = Locale.getDefault();

		initNames(monthNames);
		...
		if (hasSpinner) {
			...
			// RM R-AAG-881 08-2010 - Remuevo el combobox asi se cierra el popup del daychooser al clickear afuera del mismo.
			spinner.addChangeListener(this);
			// spinner.setEditor(comboBox);

			textMonth = new JTextField("12345678901234");
			this.setPreferredSize( textMonth.getPreferredSize());
//			textMonth.setText("DICIEMBREEE"); //R-AAG-881
//			textMonth.setSize(50, spinner.getHeight());
			spinner.setEditor(textMonth);
			textMonth.setBorder(new EmptyBorder(0, 0, 0, 0));
			// comboBox.setBorder(new EmptyBorder(0, 0, 0, 0));
			add(spinner, BorderLayout.CENTER);
//			add(comboBox,BorderLayout.SOUTH ); //R-AAG-881
			updateUI();
		} else {
	 ...
	 */
	public void initNames(String[] monthNames) {
	...
	public void stateChanged(ChangeEvent e) {
		...
		boolean increase = !(value > oldSpinnerValue) ? true : false; //R-AAG-881 // hacia abajo incremento los meses, hacia arriba decremento.
	...
	private void setMonth(int newMonth, boolean select) {
		...
		if (select) {
			comboBox.setSelectedIndex(month);
			textMonth.setText((String) comboBox.getItemAt(month));
		}
