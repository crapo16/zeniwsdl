 ...
 * @version $LastChangedDate: 2012-01-27 18:08:26 -0300 (vie, 27 ene 2012) $
 * @since 26/07/2010 Se cambio la forma de mostrar los valores con tablas en lugar de botones
 * @since 26/07/2010 Changes on the way that it renders, instead of buttons it renders through a table
 * // REQ R-AAG-726 - Cambio de botones a tabla.
 */
...
*/
	protected JPanel tableDaysPanel;
	...
	private JPanel tableWeekPanel;
	//**//
	public JDayChooser(boolean weekOfYearVisible, String[] days) {
		setDayNames(days);
		init(weekOfYearVisible);
	}
	...
	public void init(boolean weekOfYearVisible) {
		setName("JDayChooser");
...
//			add(weekPanel, BorderLayout.WEST);
			add(tableWeekPanel, BorderLayout.WEST);
		}

		add(tableDaysPanel, BorderLayout.CENTER);
	...
	private void generateDayTable() {
		tableDays      = getNewTableDays();
		tableDaysPanel = prepareTableDaysPanel();
		configureTableDays(tableDays);
		configureTablDayeHeader(tableDays.getTableHeader());
		tableDaysPanel.add(tableDays.getTableHeader(), BorderLayout.NORTH);
		tableDaysPanel.add(tableDays, BorderLayout.SOUTH);
		tableDays.addMouseListener(new MouseAdapter(){
			public void mouseClicked(MouseEvent e){
		    	 int d = Integer.parseInt(((String) (tableDays.getValueAt(tableDays.getSelectedRow(), tableDays.getSelectedColumn()) == null ? "0" : tableDays.getValueAt(tableDays.getSelectedRow(), tableDays.getSelectedColumn()))));
		    	 if (d != 0 ) firePropertyChange("day", 0, d);
		    }
	...
	private void configureTableDays(JTable tableDays2) {
		tableDays2.setModel(prepareTableDaysModel());
		tableDays2.setDefaultRenderer(Object.class, prepareDaysDefRenderer());
		tableDays2.setCellSelectionEnabled(true);
		tableDays2.setBackground( new Color(190, 200, 238) );
		if ( selectedColor == null ) selectedColor = new Color(160, 160, 160);
		tableDays2.setSelectionBackground(selectedColor);
		tableDays2.setGridColor(decorationBackgroundColor);
		for (int i = 0; i < tableDays2.getColumnModel().getColumnCount(); i++) {
			TableColumn column = tableDays2.getColumnModel().getColumn(i);
			column.setPreferredWidth(widthCell);
			column.setHeaderValue(tableDays2.getModel().getColumnName(i));
		}
		tableDays2.setRowHeight(heightCell);
		tableDays2.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		tableDays2.setDefaultEditor(Object.class, new TableCellEditor() {
	...
	private void configureTablDayeHeader(JTableHeader tableHeader) {
		tableHeader.setDefaultRenderer(new DefaultTableCellRenderer() {
			...
			public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
				...
				if (((String) value).equals("Dom") || column == 0 ) {
					setForeground(sundayForeground);
				} else {
					setForeground(weekdayForeground);
				}
				return this;
			}
		});
	...
	private TableCellRenderer prepareDaysDefRenderer() {
		return new DefaultTableCellRenderer() {
			private static final long serialVersionUID = 1L;
			public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
				super.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column);
				if (value == null) { // para que no seleccione los nulos
					setBackground( decorationBackgroundColor );
				} else {
					if (( calendar.equals(today) ) && ( Integer.parseInt( (String)value) == today.get(Calendar.DAY_OF_MONTH))) {
						setForeground(sundayForeground);
						setBackground(oldDayBackgroundColor);
					} else {
//						Para colorear domingo, pero tengo que calcularlo con los datos que me llegan arriba.
...
//						}
	...
	private JPanel prepareTableDaysPanel() {
		JPanel tmp = new JPanel();
		tmp.setLayout(new BorderLayout());
		return tmp;
	}

	private JTable getNewTableDays() {
		return new JTable() {
			private static final long serialVersionUID = 2L;
			private TableCellRenderer renderer;
//			//* Deseo poder no seleccionar nada y que siga viva la instancia de la tabla. *//
//			public void changeSelection(int row, int column, boolean toggle, boolean extend){
//				if ( getValueAt(row, column) != null) {
//					super.changeSelection(row, column, toggle, extend);
//				}
//			}
			public void setDefaultRenderer(Class<?> class1, TableCellRenderer renderer) {
	...
	private DefaultTableModel prepareTableDaysModel() {
		return new DefaultTableModel(new Object[][] {
			...
			public boolean isCellEditable(int row, int column) {
				return (super.getValueAt(row, column) == null);
			}
		};
	}
	private void generateWeekTable() {
		tableWeek      = prepareTableWeek();
		tableWeekPanel = prepareTableWeekPanel();
		configureTableWeekHeader(tableWeek.getTableHeader());
		configureTableWeek(tableWeek);
		tableWeekPanel.add(tableWeek, BorderLayout.CENTER);
		tableWeekPanel.add(tableWeek.getTableHeader(), BorderLayout.NORTH);
	}

	private JPanel prepareTableWeekPanel() {
		return prepareTableDaysPanel();
	}

	private void configureTableWeek(JTable table) {
		table.setModel(prepareTableWeekModel());
		table.setDefaultRenderer(Object.class, prepareWeekDefRenderer());
		table.setCellSelectionEnabled(false);
		table.setGridColor(decorationBackgroundColor);
		table.setBackground( decorationBackgroundColor );
		for (int i = 0; i < table.getColumnModel().getColumnCount(); i++) {
			TableColumn column = table.getColumnModel().getColumn(i);
			column.setPreferredWidth(widthCell);
			column.setHeaderValue(table.getModel().getColumnName(i));
		}
		table.setRowHeight(heightCell);
	...
	private TableCellRenderer prepareWeekDefRenderer() {
		return new DefaultTableCellRenderer() {
			private static final long serialVersionUID = 1L;
			public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
				super.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column);
	...
	private void configureTableWeekHeader(JTableHeader tableHeader) {
		tableHeader.setDefaultRenderer(new DefaultTableCellRenderer() {
			private static final long serialVersionUID = 1L;
			public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
				super.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column);
	...
	private JTable prepareTableWeek() {
		return getNewTableDays();
	}

	private DefaultTableModel prepareTableWeekModel() {
		return new DefaultTableModel(new Object[][] {
				{ null },
				{ null },
				{ null },
	...
	private void drawDayNames() {
		...
		if (dayNames == null) {
			...
			// getShortWeekdays() is 1-Based, but dayNames is 0-Based like most of REAL vectors.
			for (int i = 0; i < 7; i++){
...
//			days[i].setText(dayNames[i]);
			//R-AAG-881 // day is index of dayNames, i, index of column title.
			tableDays.getColumnModel().getColumn(i).setHeaderValue(dayNames[day-1]);
...
//			weeks[i].setText(buttonText);
			tableWeek.setValueAt(buttonText, i-1, 0);
			if ((i == 5) || (i == 6)) {
//				weeks[i].setVisible(days[i * 7].isVisible());
//				weeks[i].setVisible((tableDays.getValueAt( i - 1, 0)) != null );
				tableWeek.setValueAt( ((tableDays.getValueAt( i - 1, 0)) != null) ? buttonText : null, i-1, 0);
			}
		}
	}
...
//			days[i + 7].setText("");
			tableDays.setValueAt( null, i/7, i%7 );//Pongo en cero
		}
...
//			days[i + n + 7].setVisible(true);
			tableDays.setValueAt( Integer.toString(n + 1), ((n+i)/7 ),  ((n+i)%7)  );//TODO agregar un mod despues!
...
//				selectedDay = days[i + n + 7];
				tableDays.getSelectionModel().setSelectionInterval(((n+i)/7 ), ((n+i)/7 ));
				tableDays.getColumnModel().getSelectionModel().setSelectionInterval(((n+i)%7), ((n+i)%7));
			} else {
...
//			days[k].setText("");
			tableDays.setValueAt( null, (k-7)/7, (k-7)%7);//Pongo en cero
		}
	...
	public void setDay(int d, boolean property) {
		if (d < 1) {
			d = 1;
		}
		Calendar tmpCalendar = (Calendar) calendar.clone();
...
//			if (days[i].getText().equals(Integer.toString(day))) {
			if ( 
					(tableDays.getValueAt(((i-7)/7), ((i-7)%7)) == null ? false : Integer.parseInt((String)tableDays.getValueAt(((i-7)/7), ((i-7)%7)))==day)) {
//				selectedDay = days[i];
//				selectedDay.setBackground(selectedColor);
				tableDays.getSelectionModel().setSelectionInterval(((i-7)/7 ), ((i-7)/7 ));
				tableDays.getColumnModel().getSelectionModel().setSelectionInterval(((i-7)%7), ((i-7)%7));
				break;
			}
		}
	...
	public void setMonth(int month) {
		...
		if (maxDays == day || ( maxDays > calendar.getActualMaximum(Calendar.DAY_OF_MONTH) && calendar.get(Calendar.MONTH) == Calendar.FEBRUARY )) {
...
//		}
	 ...
	 */
	/*
	 * Code below commented out by Mark Brown on 24 Aug 2011. This code breaks
