...
//					querySBuilder.append("AND c.vendedor_id    in ").append(cuentas_cuentas).append(opcionesFiltro);
            ...
            // #1516
            String sqldetalle = " select 'Multa por incumplimiento: ' || con.multaporincumplimiento   || ' - Fijaciones: ' || grpfijadesc.detalle_fijacion detalle_fijacion  from contrato con   inner join (  select  ' Cantidad Minima Diaria: ' || grp_fija.CANTIDADMINIMADIARIA || ' Canitdad Maxima Diaria: ' ||grp_fija.CANTIDADMAXIMADIARIA ||  ' Cantidad Minima Semanal: ' || grp_fija.CANTIDADMINIMASEMANAL || ' Cantidad Maxima Semanal: ' || grp_fija.CANTIDADMAXIMASEMANAL ||    ' Cantidad Minima Mensual: ' || grp_fija.CANTIDADMINIMAMENSUAL || ' Cantidad Maxima Mensual: ' || grp_fija.CANTIDADMAXIMAMENSUAL ||  ' Fecha Desde: ' || to_char(grp_fija.FECHAFIJACIONDESDE,'DD/MM/YYYY') || ' Fecha Hasta: ' || to_char(grp_fija.FECHAFIJACIONHASTA,'DD/MM/YYYY') ||  ' Horario Fijacion: ' || grp_fija.HORARIOFIJACION || ' Moneda: ' || moneda.nombre || ' Unidad de Medida: ' || udmedida.nombre ||  ' Pizarra: ' || mercado.nombre  detalle_fijacion, contrato_id  from grupo_fijaciones grp_fija  inner join moneda moneda on (moneda.id=grp_fija.moneda_id)  inner join unidad_de_medida udmedida on (udmedida.id=grp_fija.unidaddemedida_id)  inner join ponderado_producto_mercado ppm on (ppm.grupofijaciones_id=grp_fija.id)  inner join mercado mercado on (mercado.id = ppm.mercado_id)  where grp_fija.fechabaja is null  and ppm.fechabaja is null  ) grpfijadesc on (grpfijadesc.contrato_id=con.id) ";

            // #1548
            String sqlajustes = " select ' Mercado: ' || mercado.nombre  || ' Tipo: ' || taf.nombre ||  ' Moneda: ' || moneda.simbolo  || ' Porcentaje: ' || to_char(vaf.PORCENTAJEAJUSTEFIJACION)   || ' Importe: ' || to_char(vaf.IMPORTEAJUSTEFIJACION) descripcion  from valor_ajuste_fijacion vaf  inner join grupo_fijaciones grp_fija on (grp_fija.id = vaf.grupofijaciones_id)  inner join moneda moneda on (moneda.id=grp_fija.moneda_id)  inner join unidad_de_medida udmedida on (udmedida.id=grp_fija.unidaddemedida_id)  inner join ponderado_producto_mercado ppm on (ppm.grupofijaciones_id=grp_fija.id)  inner join mercado mercado on (mercado.id = ppm.mercado_id)   inner join TIPO_AJUSTE_FIJACION taf on (taf.id = vaf.tipoajustefijacion_id)  where grp_fija.fechabaja is null   and ppm.fechabaja is null   and vaf.fechabaja is null ";

            // #1556
            String sqlcomentario = " select  comentario from comentario_grupo_fijaciones cgf  inner join grupo_fijaciones grp_fija on (grp_fija.id = cgf.grupofijaciones_id)  where cgf.fechabaja is null  and grp_fija.fechabaja is null ";

            for (final ContratoType contrato : _returnContratos) {
                String sql = sqldetalle + " where con.id = " + contrato.getContratoId();
                // #1548
                String sqlaj = sqlajustes + " and grp_fija.contrato_id = " + contrato.getContratoId();

                // #1556
                String sqlcom = sqlcomentario + " and grp_fija.contrato_id = " + contrato.getContratoId();
    ...
    public ArrayOfCobranzaType obtenerCobranzas(AuthType auth, ArrayOfIdCuenta acuenta, FechaTimeType fechaTope,
        ...
        try {
            ...
            if (acuenta == null || acuenta.getCuentas() == null || acuenta.getCuentas().size() <= 0) {
                throw new ZeniServerExeption("Los id de la cuenta no puede ser nulo o vacio, Cuenta:" + acuenta,
                        FAULTCONSTANTS.ERROR_ERRONEO);
            }
    ...
    public InformacionCuentaCorrienteType obtenerInformacionCuentaCorriente(AuthType auth, ArrayOfIdCuenta cuentas,
        ...
        try {
            ...
            String querySBuilder
                    = // #1698
                    // .append(listaCuentas)
                    // #1698
                    // .append(listaCuentas)
                    "SELECT com.idcuenta idcuenta, com.denominacion denominacion, com.nroCuenta nroCuenta, com.idcomp, com.contratid, TO_CHAR(com.fecha, 'yyyy-MM-dd') fecha, com.ridx, com.tipocom, TO_CHAR(com.fechavalor, 'yyyy-MM-dd') fechavalor, com.tipocom, com.comprobante, com.numerocontrato numerocontrato, com.descripcion, com.importe, com.saldo saldoSinSuma, saldoinicialcalculo.saldoinicial saldoinicialSinSuma, TO_CHAR(to_number(TO_CHAR(com.saldo)) + to_number(TO_CHAR(saldoinicialcalculo.saldoinicial))) saldoSuma2 FROM (SELECT cte.id idcuenta, cte.denominacioncuenta denominacion, cte.nrocuenta nrocuenta, cm.fecha, cm.fechavalor, DECODE(tc.abreviatura,'RT',cm.id,cm.ref_id) idcomp, tc.abreviatura tipocom, tc.abreviatura ||' ' ||cm.letra ||' ' ||cm.pvta ||' ' ||cm.nro comprobante, c.numerocontrato, c.id contratid, cm.descripcion, DECODE('" + (filtroDeMovimiento.equals(EnumDeMovimientoType.PENDIENTE) ? "SALDOPENDIENTE" : "IMPORTE") + "','IMPORTE', cm.importe, cm.saldopendiente) importe, " + "SUM(DECODE('" + (filtroDeMovimiento.equals(EnumDeMovimientoType.PENDIENTE) ? "SALDOPENDIENTE" : "IMPORTE") + "','IMPORTE', cm.importe, cm.saldopendiente)) over (order by cm.fecha ASC, cm.id ASC) saldo, " + "SUM(1) over (order by cm.fecha ASC, cm.id ASC) ridx " + "FROM ctacte_mov cm " + "INNER JOIN tipo_comprobante tc " + "ON (tc.id=cm.tipocomprobante_id) " + "INNER JOIN ctacte_cuenta cc " + "ON (cc.id=cm.cuenta_id) " + "INNER JOIN cuenta_cliente cte  " + "ON (cte.id=cc.cuentacliente_id) " + "LEFT OUTER JOIN contrato c " + "ON (c.id               =cm.contrato_id) " + "WHERE cm.fechabaja    IS NULL " + "AND cm.moneda_id       = " + filtroDeMoneda.getId() + " " + "AND cc.cuentacliente_id in " + " (CUENTAIDXX) " + " " + "AND TRUNC(fecha)      >= to_date('" + DateUtil.ToString.yyyyMMdd(rangoDeFechas.getDesde()) + "','yyyy-MM-dd') " + "AND TRUNC(fecha)      <= to_date('" + DateUtil.ToString.yyyyMMdd(rangoDeFechas.getHasta()) + "','yyyy-MM-dd')  " + (filtroDeMovimiento.equals(EnumDeMovimientoType.PENDIENTE) ? "AND cm.saldopendiente <>0 "
                    : " ORDER BY cm.fecha ASC, cm.id ASC ") + ") com, " + "(SELECT NVL(SUM(NVL(DECODE('" + (filtroDeMovimiento.equals(EnumDeMovimientoType.PENDIENTE) ? "SALDOPENDIENTE" : "IMPORTE") + "','IMPORTE', NVL(cm2.importe, 0), NVL(cm2.saldopendiente, 0)), 0)), 0) saldoInicial " + "FROM ctacte_mov cm2 " + "INNER JOIN ctacte_cuenta cc2 " + "ON (cc2.id              =cm2.cuenta_id " + "AND cc2.fechabaja      IS NULL) " + "WHERE cm2.fechabaja    IS NULL " + "AND cm2.fecha           <to_date('" + DateUtil.ToString.yyyyMMdd(rangoDeFechas.getDesde()) + "','yyyy-MM-dd') " + "AND cc2.cuentacliente_id in " + " (CUENTAIDXX) " + " " + "AND cm2.moneda_id       = " + filtroDeMoneda.getId() + " " + ") saldoinicialcalculo ORDER BY com.denominacion,com.ridx ASC  ";
            final ResulsetObjectBuilder resb = new ResulsetObjectBuilder() {
                @Override
                public void thisIsTheResulset(ResultSet rset) throws SQLException {
                    while (rset.next()) {
            ...
            for (int i = 0; i < cuentas.getCuentas().size(); i++) {
                ...
                String sql = querySBuilder.replaceAll("CUENTAIDXX", idcuenta);
            ...
            for (int i = 0; i < cuentas.getCuentas().size(); i++) {
                ...
                for (int f = 0; f < client.getCuentas().getCuentas().size(); f++) {
                    if (client.getCuentas().getCuentas().get(f).getId().equals(cuenta)) {
                        ...
                        String querySBuilder2 = "SELECT max(cc.id) cuenta_id, max(cc.tipocuenta) tipocuenta, max(cc.moneda_id) moneda_id, max(cc.saldo) saldo, nvl(sum(cm.importe),0) saldoAnterior, max(cc.saldous) saldous, to_char(max(cc.fechacreacion), 'yyyy-MM-dd') fechacreacion, '2012-04-01' fechasaldoanterior FROM ctacte_mov cm INNER JOIN ctacte_cuenta cc ON (cc.id              =cm.cuenta_id AND cc.fechabaja      IS NULL) WHERE cm.fechabaja    IS NULL AND cm.fecha      <to_date('" + DateUtil.ToString.yyyyMMdd(rangoDeFechas.getDesde()) + "','yyyy-MM-dd') " + "AND cc.cuentacliente_id= " + cuenta + " " + "AND cm.moneda_id       = " + filtroDeMoneda.getId() + " ";
                        ...
                        ZeniQueryExcecutor.excecuteSelect(querySBuilder2, resb2);
